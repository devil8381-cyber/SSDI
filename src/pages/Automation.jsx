...
export default function Automation() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div>
        <h1 className="text-xl font-bold text-slate-100">Automation</h1>
        <p className="text-sm text-slate-400">Lead distribution and pool hygiene — set it and forget it.</p>
      </div>
      ...
      <div className="card p-6">
        <div className="mb-4 flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm font-bold text-slate-100">Auto-assign new leads</h2>
            <p className="text-xs text-slate-400">Applies to Meta leads, CSV imports, and manual adds that arrive without an agent.</p>
          </div>
        </div>
        <div className="grid gap-3 md:grid-cols-2">
          <button
            ...
            disabled={savingMode}
            className={`rounded-xl border p-4 text-left transition ${mode === 'off' ? 'border-brand-500 bg-brand-500/10 ring-2 ring-brand-100' : 'border-slate-700 hover:border-brand-300'}`}
          >
            <p className="text-sm font-semibold text-slate-100">⏸ Off — manual distribution</p>
            <p className="mt-1 text-xs text-slate-400">New leads land in the unassigned pool. You assign them from the Leads page (individually or in bulk).</p>
          </button>
          <button
            onClick={() => saveMode('round_robin')}
            disabled={savingMode}
            className={`rounded-xl border p-4 text-left transition ${mode === 'round_robin' ? 'border-brand-500 bg-brand-500/10 ring-2 ring-brand-100' : 'border-slate-700 hover:border-brand-300'}`}
          >
            <p className="text-sm font-semibold text-slate-100">⚡ Round-robin</p>
            <p className="mt-1 text-xs text-slate-400">Every new lead is auto-assigned to the active agent with the fewest leads. No babysitting required.</p>
          </button>
        </div>
      </div>
      ...
      <div className="card p-6">
        <div className="mb-4 flex items-center gap-2.5">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-amber-500/100 text-white"><Repeat size={18} /></div>
          <div>
            <h2 className="text-sm font-bold text-slate-100">Lead recycling</h2>
            <p className="text-xs text-slate-400">Finds leads nobody has touched in a while (excluding Signed/Approved) and returns them to the unassigned pool so someone fresh can work them.</p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <label className="text-sm text-slate-300">No activity for</label>
          ...
          <div className="mt-4 rounded-xl bg-slate-800/70 p-4">
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {DISPOSITIONS.filter((d) => preview.byDisposition[d]).map((d) => (
                <span key={d} className="rounded-md bg-slate-900 px-2 py-0.5 text-xs text-slate-300 ring-1 ring-slate-700">
  ...
  return (
    <div className="card p-6">
      <div className="mb-4 flex items-center gap-2.5">
        ...
        <div>
          <h2 className="text-sm font-bold text-slate-100">Follow-up rules</h2>
          <p className="text-xs text-slate-400">When an agent sets one of these dispositions, ABA automatically creates the next task — so no lead is ever forgotten.</p>
        </div>
      </div>
      {!rules ? <p className="py-4 text-center text-sm text-slate-400">Loading…</p> : (
        <div className="space-y-2">
          {rules.map((r, i) => (
            <div key={i} className="flex flex-wrap items-center gap-2 rounded-lg bg-slate-800/70 p-2.5">
              ...
              <button className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-700 hover:text-rose-500" onClick={() => setRules((rs) => rs.filter((_, x) => x !== i))} title="Remove rule"><Trash2 size={14} /></button>
            </div>
          ))}
  ...
  return (
    <div className="card p-6">
      <div className="mb-4 flex items-center gap-2.5">
        ...
        <div>
          <h2 className="text-sm font-bold text-slate-100">Daily targets (per agent)</h2>
          <p className="text-xs text-slate-400">Agents see progress bars on their dashboard. Set 0 to hide a metric.</p>
        </div>
      </div>
  ...
  return (
    <div className="card p-6">
      <div className="mb-3 flex items-center gap-2.5">
        ...
        <div>
          <h2 className="text-sm font-bold text-slate-100">Welcome email on assignment</h2>
          <p className="text-xs text-slate-400">When a lead is assigned to an agent, the claimant automatically gets a welcome email with the agent's name and phone number (once per lead + agent).</p>
        </div>
      </div>
      <div className="flex items-center gap-3">
        ...
        <span className={`text-sm font-medium ${enabled ? 'text-emerald-400' : 'text-slate-400'}`}>{enabled ? 'ON' : 'OFF'}</span>
...
function CallingAppCard() {
  ...
  const { run: save, busy } = useAction(async () => {
    const saved = await api('/settings/call', { method: 'PUT', body: cfg })
    setCallConfig(saved) // every phone link picks up the new app instantly — no reload
  ...
  return (
    <div className="card p-6">
      <div className="mb-3 flex items-center gap-2.5">
        ...
        <div>
          <h2 className="text-sm font-bold text-slate-100">Calling app (click-to-call)</h2>
          <p className="text-xs text-slate-400">Which app opens when an agent clicks a phone number anywhere in the CRM.</p>
        </div>
      </div>
      <div className="space-y-2">
        {radios.map((r) => (
          <label key={r.v} className={`flex cursor-pointer items-start gap-3 rounded-xl border p-3 ${cfg.scheme === r.v ? 'border-brand-500 bg-brand-500/10' : 'border-slate-700'}`}>
